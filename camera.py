...
# Streamlit app
def main():
    st.title("Live Waste Classification and Disposal Guidance in India")

    # Option to select live camera feed
    use_live_camera = st.checkbox("Use Live Camera")
    ...
    if use_live_camera:
        ...
        # Set up a button to take a picture
        ...
        while cap.isOpened():
            ...
            # Check if the "Take Picture" button is clicked
            if take_picture_button:
                st.write("Taking picture in 1 second...")
                time.sleep(1)  # Wait for one second
                captured_frame = frame.copy()  # Capture the current frame
                st.write("Picture taken.")
                break  # Exit the loop to process the captured frame

        # Process the captured frame if available
        if captured_frame is not None:
            img_pil = Image.fromarray(captured_frame)
            st.image(captured_frame, caption='Captured Image', use_column_width=True)

            # Preprocess the image and classify it
            processed_image = load_and_preprocess_image(img_pil)
        ...
        st.write("Enable 'Use Live Camera' to start the live classification.")
